...
Uses OpenAI Realtime for speech and Stream Edge for transport.
Loads STREAM_* and OPENAI_API_KEY from the parent .env.local (or .env) and vision-agent/.env.
...
def load_env() -> None:
    """Load Stream/OpenAI keys from parent repo (Expo uses .env.local) and local overrides."""
    for path in (
        ROOT_DIR / ".env",
        ROOT_DIR / ".env.local",
        AGENT_DIR / ".env",
...
async def create_agent(**kwargs) -> Agent:
    """Factory: voice-only teacher with OpenAI Realtime on Stream Edge."""
    return Agent(
        edge=getstream.Edge(),
...
def _friendly_join_error(error: Exception) -> str:
    ...
    if "insufficient_quota" in message or "exceeded your current quota" in message:
        return (
            "OpenAI API quota exceeded. Add billing or credits at "
...
async def join_call(agent: Agent, call_type: str, call_id: str, **kwargs) -> None:
    ...
    try:
        ...
        logger.info(
            "Joining call %s (%s) — lesson: %s, language: %s, learner: %s, goals: %s",
            call_id,
            call_type,
            lesson_title,
            language_name,
